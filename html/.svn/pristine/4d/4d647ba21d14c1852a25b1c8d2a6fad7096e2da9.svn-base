	...
	*******************/
	...
	OP_Tools.StaffSelector.prototype = {
		...
		createEmailList : function($dom){			
			var arr = this.getEmailOnPop(), _htm = '';
			// console.log(arr);
			for(var i=0, k=arr.length; i<k; i++){
		...
		//获得弹层里的已选数据,封装成arrObj
		getEmailOnPop : function(){
			var lis = this.selectbox.find('li'),
				arr = [];
			for(var i=0,k=lis.length; i<k; i++){
				var bs = $(lis[i]).find('b'),
					bool = $(bs[2]).attr('class').indexOf('fcr') !== -1 ? true : false,
					_obj = {
						name : bs[0].title,
						empCode : bs[0].getAttribute('data-uid'),
						deptName : bs[1].title,						
						turnRed : bool				
					}
				_obj[this.kType] = bs[2].title;
				arr.push(_obj);
			}
			return arr;
		},		
		
		//获得box里以有的数据,封装成arrObj
		...
		activeOnEmailBox : function(boxClass, listObj){
			...
			$(document).on('keyup', '.data_text', function(e){
				...
				case 8 :    //退格
					...
					break;
				case 65 :   //ctrlKey
					if(e.ctrlKey) $(this).parent().prevAll().addClass('activ_cell');
					break;
				default :
					return false;
				}
			...
			$(document).on('dblclick', '.list_cell', function () {
				$(this).removeClass('activ_cell').find('.listen_cell').focus();
				this.contentEditable = true;
				return false;
			});
			
			//单击地址列表时，添加listen_cell，并高亮显示
			$(document).on('click', '.list_cell', function(e){
				if(!e.shiftKey && !e.ctrlKey) $(this).siblings('.list_cell').removeClass('activ_cell');  //如果没有按shift,则一次只能选择一个
				if(this.contentEditable === 'inherit' || this.contentEditable !== 'true'){   //当处于编辑状态时，就不响应接下来的操作
					$(this).addClass('activ_cell');
					var _lis = $(this).find('.listen_cell');
					if(!_lis[0]){
						_lis =$( '<input type="text" class="listen_cell" />');
						$(this).append(_lis)
					}					
					$(this).find('.listen_cell').focus();	
				}			
				return false;
			});			
			...
			$(document).on('keyup', '.listen_cell', function(e){			
				var _cod = e.keyCode;				
				if(_cod == 46 || _cod == 110 || _cod == 8 ){  				
					var _cell = $(this).parent(),
						_cels = _cell.siblings('.activ_cell');
					_cell.remove();
					_cels.remove();  //删掉所有选中的
				}	
			});
			//取消冒泡以保持当前地址模块的可编辑状态
			$(document).on('blur', '.listen_cell', function(e){			
				return false;
			});
			...
			$(document).on('blur', '.list_cell', function(){
				if(this.contentEditable === 'inherit' || this.contentEditable === 'true') this.contentEditable = false;				
		...
		getSelectedNum : function($dom){
			var _num = this.selectbox.find('li').length;
			$dom.html(_num)
			return _num;
		},
		
		//鼠标框选
		addDropSelect : function($box){
			...
			$box.on('mouseup', function(){
				var _d = getSelectedContents(),
					dom = $(_d);
				dom.each(function(i,d){
					if(d.nodeType === 1){
						$(d).addClass('on');
					}
				});
	...
	OP_Tools.inputVerify.prototype = {
		...
		checkContent : function (inputs, type) {
			var that = this, 
				_event = type === 'cn' ? 'blur' : 'keyup afterpaste';
			inputs.on(_event, function () {
